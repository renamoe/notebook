...
[link](https://codeforces.com/contest/1238)

### \*D. AB-string

对于一个非边界的字符，讨论一下前后两个字符的可能情况，发现无论如何都会被回文子串覆盖。

那么不被覆盖只可能是边界字符，容易得到只有 $\texttt{ABBBBB}$, $\texttt{BBBBBA}$ 这样只有一端端头字符不同的不合法。扫一遍即可。

### \*E. Keyboard Purchase

将枚举全排列优化为状压的套路，即状压已加入点的集合。

把字母之间移动距离拆开，考虑每个相邻字母之间部分的贡献，也就是已加入集合和未加入集合的点之间两两距离之和。这个代价函数可以 $\mathcal O(2^nn)$ 递推。

另一种思路是把距离拆为两个坐标之差。

### F. The Maximum Subtree

基于区间的性质，和一个区间满足相交却不包含关系的至多有两个区间，但是可以有无限多包含的小区间。

在树上也就是要找一条链满足链上所有点邻居集合（包括自身）的并集最大。类似书的直径进行 DP，维护每个点向下延伸的最优链，在 LCA 处合并，复杂度 $\mathcal O(n)$。

### \*G. Adilbek and the Watering System

总感觉在哪里见过的贪心题，好像是叫走夜路。

某一时刻到达一个人，先贪心的加满，如果还有剩的，那么把之前加入的未花费并且价格比当前高的部分撤销（按价值从大到小的顺序），改为当前的价格。

那么要维护当前水箱每种价格分别有多少水，从前往后每次优先花费价格最小的，可能去掉价格最大的，用 $\texttt{std::map<>}$ 维护，复杂度 $\mathcal O(n\log n)$。

## EDU75

[link](https://codeforces.com/contest/1251)

### D. Salary Changing

二分中位数，需要注意二分的左边界是 $\{l_i\}$ 的中位数。

然后 check总和不超过 $s$（贪心往小赋值）的情况下，是否能存在不少于 $(n+1)/2$ 个数权值 $\ge \mathrm{mid}$。只要找出 $(n+1)/2$ 个权值为 $\max(l_i,\mathrm{mid})$，其余全部赋为 $l_i$ 即可。

具体的，将区间分成三类：$l_i\ge \mathrm{mid}$ 的是排名必须选的；然后从剩余的满足 $r_i\ge \mathrm{mid}$ 的选 $l_i$ 最大的若干个，需要提前排序；$r_i < \mathrm{mid}$ 的都不选。复杂度 $\mathcal O(n\log V)$。

### \*E2. Voting (Hard Version)

按已有人数从小到大考虑，把所有人按 $m_i$ 分组。对于所有 $m_i=x$ 的人，如果要得到这些人，已有人数要不少于 $x$，即所有 $m_i<x$ 的人数 + 购买的人数 $\ge x$，那么从所有 $m_i\ge x$ 的人中贪心选取 $p_i$ 最小的购买。

具体实现，$x$ 从大到小，用小根堆维护未购买过的人即可。

### \*F. Red-White Fence

红板可以直接枚举，那么每次询问限制的就是水平线段长度和，即白板的个数。

先考虑白板长度互不相同的情况，那么只要讨论每个白板是否选、放在左边还是右边，答案为 $\binom{n}{m}2^m$。

如果每个长度都有 $\ge 2$ 个白板，至多情况下两边是可以都放一个的。用两个元素来表示，答案即 $\binom{2n}{m}$。可以理解为任意一对如果第一个被选则表示左边放一个，第二个被选表示右边放一个，两个都被选表示两边都放。

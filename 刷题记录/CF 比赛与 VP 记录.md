...
[link](https://codeforces.com/contest/1588)

### A. Two Arrays

排序后逐位比较，$b_i-a_i\in \{0,1\}$。或者放到桶上搞搞。

### B. Guess the Permutation

$40$ 次意味着只允许一次二分。

一个翻转后的区间 $[l,r]$ 逆序对数为 $\frac{1}{2}\mathrm{len}(\mathrm{len}-1)$，查询一次 $[l,r-1]$ 的逆序对数即可得到 $\mathrm{len}$。那么只需要二分一次找到右边界。

### C. Game with Stones

操作顺序不影响。一个序列 $a_1\dots a_n$ 合法的条件为，一定可以表示为 $b_1,b_1+b_2,b_2+b_3,\dots,b_{n-2}+b_{n-1},b_{n-1}$，要求 $b_i\ge 0$。

用 $a_i$ 表示 $b_i$ 就是 $a_1,a_2-a_1,a_3-a_2+a_1,\dots$。并且最后一个数为 $0$。

为了统计区间数，将其差分为前缀的奇数项减偶数项（或偶数项减奇数项），设为 $f_i$。枚举左端点 $l$，分下标奇偶性分别找到第一个不合法的位置 $f_i < f_l$，求这个范围里有多少个 $b_j$ 为 $0$ 的数即可。前者可以树状数组，后者可以离线处理。

更优美的写法当然是有的。按 $f_i$ 值分组考虑，用 $\texttt{std::set<>}$ 维护不合法的集合，不合法的点会将序列分为若干段，同一段内相同值的点可以两两构成合法答案。从小到大，按下标奇偶性决定从集合中插入还是删除。

复杂度都是 $\mathcal O(n\log n)$。

### \*D. Strange LCS

DP 需要记录在 $n$ 个串中分别匹配到的位置。因为这 $n$ 个位置一定是相同字符，而一个串中一个字符至多出现 $2$ 次，只需要记录是第一个还是第二个，那么可以二进制状压。
